...
import woo.sopt22.meowbox.View.MyPage.ProgressBar.StateItemDescription;
import woo.sopt22.meowbox.View.MyPage.ProgressBar.StateItemNumber;
import woo.sopt22.meowbox.View.MyPage.ProgressBar.OnStateItemClickListener;
import woo.sopt22.meowbox.View.MyPage.ProgressBar.FontManager;
...
public class StateProgressBar extends View {
    ...
    private void init(Context context, AttributeSet attrs, int defStyle) {
        ...
        if (attrs != null) {
            ...
            mCurrentStateNumber = a.getInteger(R.styleable.StateProgressBar_spb_currentStateNumber, mCurrentStateNumber)+1;
            mMaxStateNumber = a.getInteger(R.styleable.StateProgressBar_spb_maxStateNumber, mMaxStateNumber)+1;
...
//            canvas.drawLine(mCellWidth * (i + 1) - (mCellWidth / 2) - mStateRadius, mCellHeight / 2,
//                    mCellWidth * (i + 1) - (mCellWidth / 2) + mStateRadius,mCellHeight / 2, paint);
//            canvas.drawLine(mCellWidth * (i + 1) - (mCellWidth / 2) - mStateRadius, mCellHeight / 2,
//                    mCellWidth * (i + 1) - (mCellWidth / 2) + mStateRadius,mCellHeight / 2, paint);
//        }
    }
        ...
        if (!mStateDescriptionData.isEmpty()) {
            ...
            for (int i = 0; i < mStateDescriptionData.size(); i++) {
                if (i < mMaxStateNumber) {
                    innerPaintType = selectDescriptionPaint(mCurrentStateNumber-1, i);
                    ...
                    if (mIsDescriptionMultiline && mMaxDescriptionLine > 1) {
                        ...
                        for (String line : stateDescriptionLines) {
                            ...
                            if (nextLineCounter <= mMaxDescriptionLine) {
                                ...
                                canvas.drawText(line, newXPos == 0 ? xPos+mCellWidth : newXPos+mCellWidth, yPos, innerPaintType);
                            }
                        ...
                        canvas.drawText(mIsStateNumberDescending ? mStateDescriptionData.get(mStateDescriptionData.size() - 1 - i) : mStateDescriptionData.get(i), xPos+mCellWidth, yPos, innerPaintType);
                    }
    ...
    private void drawStateNumberText(Canvas canvas, int noOfCircles) {
        ...
        boolean isChecked;
        Paint paintTmpCheck;
        Paint paintTmpUnCheck;

        for (int i = 0; i < noOfCircles; i++) {

            innerPaintType = selectPaintType(mCurrentStateNumber, i, mCheckStateCompleted);
            paintTmpCheck = setPaintAttributes(4.0f, mForegroundColor);
            paintTmpUnCheck = setPaintAttributes(4.0f, mBackgroundColor);
